...
vec3 calcLineLight(LineLight light, vec3 normal, vec3 viewDir)
{
    ...
    // 颜色
    vec3 color = vec3(texture(material.diffuse0, texCoords));
    // 环境光分量
    vec3 ambient = light.ambient * color;
    // 漫反射分量
    vec3 diffuse = light.diffuse * diffScale * color;
    // 反射分量
    vec3 specular = light.specular * specScale * vec3(texture(material.specular0, texCoords));
...
vec3 calcPointLight(PointLight light, vec3 normal, vec3 viewDir, vec3 fragPos)
{
    ...
    vec3 color = vec3(texture(material.specular0, texCoords));
    // 计算环境光分量
    vec3 ambient = light.ambient * color;
    // 计算漫反射光分量
    vec3 diffuse = light.diffuse * diffScale * color;
