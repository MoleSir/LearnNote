...
int main()
{
    ...
    // 创建着色器程序
    opengl::Shader shader("./src/22-advancedGLSL/shader/vs.glsl", "./src/22-advancedGLSL/shader/fs.glsl");
    opengl::Shader skyboxShader("./src/22-advancedGLSL/shader/cbuemapVs.glsl", "./src/22-advancedGLSL/shader/cubemapFs.glsl");
    ...
    unsigned int cubemapTexture = opengl::loadCubemapFromFiles(faces, false);
    ...
    // 找到两个着色器程序中的 Matrices Uniform 块索引
    unsigned int uniformBlock    = glGetUniformBlockIndex(shader.id(), "Matrices");
    unsigned int skyUniformBlock  = glGetUniformBlockIndex(skyboxShader.id(), "Matrices"); 

    // 将这个两个 Unifrom 块都绑定到 0 好绑定点
    glUniformBlockBinding(shader.id(), uniformBlock, 0);
    glUniformBlockBinding(skyboxShader.id(), skyUniformBlock, 0);
